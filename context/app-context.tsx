...
const initialRequests: TravelRequest[] = [
  {
    ...
    reason: "Client meeting with Petrobras",
    status: "pending",
    selectedOption: {
      id: "opt-1",
      provider: "LATAM",
  ...
  },
  {
    ...
    rejectionReason: "Use company car available at destination. Contact local office.",
    approverId: "2",
  },
  {
    id: "req-004",
    ...
    reason: "Government contract negotiation",
    status: "pending",
    selectedOption: {
      id: "opt-10",
      provider: "LATAM",
...
const initialNotifications: Notification[] = [
  {
    ...
    message: "Your hotel booking at Marriott Copacabana has been approved. Approval code: APR-2026-001",
  ...
  },
  {
    ...
    message: "Your car rental request was rejected. Reason: Use company car available at destination.",
  ...
  },
  {
    ...
    userId: "2",
    type: "new_request",
    title: "New Request for Analysis",
    message: "João Silva submitted a flight request to Rio de Janeiro requiring your approval.",
  ...
  },
  {
    ...
    message: "The January 2026 travel expense report is now available for review.",
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const addRequest = useCallback((request: Omit<TravelRequest, "id" | "createdAt" | "status">) => {
    ...
    const newNotifications: Notification[] = approversAndAdmins.map((user) => ({
      ...
      type: "new_request",
      title: "New Request for Analysis",
      message: `${request.userName} submitted a ${request.type} request to ${request.destination} requiring approval.`,
    ...
    (requestId: string, approverId: string) => {
      ...
      if (request) {
        const newNotification: Notification = {
          ...
          type: "approval",
          title: "Request Approved",
          message: `Your ${request.type} request to ${request.destination} has been approved. Code: ${approvalCode}`,
          date: new Date().toISOString(),
          read: false,
          requestId,
        }
    ...
    (requestId: string, approverId: string, reason: string) => {
      ...
      if (request) {
        const newNotification: Notification = {
          ...
          type: "rejection",
          title: "Request Rejected",
          message: `Your ${request.type} request to ${request.destination} was rejected. Reason: ${reason}`,
          date: new Date().toISOString(),
          read: false,
          requestId,
        }
